...
function renderCommitments(){
  ...
  const all = (db.commitments||[]).slice().sort((a,b)=>Number(b.amount)-Number(a.amount));
  const list = all.slice(0, commitmentsListShowCount);
  ...
  const more = all.length>list.length ? `<button class="btn sm outline" style="margin-top:6px;" onclick="showMoreCommitments()">⬇️ عرض المزيد (${all.length-list.length} متبقي)</button>` : '';
  box.innerHTML = rows + more;
}
